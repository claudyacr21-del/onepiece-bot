...
async function resolveRole(guild, config) {
  ...
  if (config.roleId) {
    ...
    console.warn(
      `[ARENA RANK ROLES] ${config.env} is set but role was not found.`
    );
  }
...
async function syncArenaRankRoles(client, preferredGuild = null) {
  ...
  const topRankHolders = getArenaTop3();
  ...
  for (const holder of topRankHolders) {
    desiredRankByUserId.set(
      String(holder.userId || holder.id),
      Number(holder.rank)
    );
  }
  ...
  console.log(
    ...
    topRankHolders
      .map((entry) => `${entry.username}#${entry.rank}:${entry.points}`)
      .join(" | ") || "none"
  );
  ...
  for (const guild of guilds) {
    ...
    for (const config of configs) {
      ...
      if (role.position >= me.roles.highest.position) {
        console.warn(
          `[ARENA RANK ROLES] Bot role must be above ${role.name}`
        );
        continue;
      }
    ...
    for (const userId of desiredRankByUserId.keys()) {
    ...
    for (const member of guild.members.cache.values()) {
      ...
      const desiredRole = desiredRank
        ? resolvedRoles.find(
            (entry) => Number(entry.rank) === Number(desiredRank)
          )
        : null;
      ...
      for (const roleId of allRankRoleIds) {
        ...
        if (hasRole && !shouldHave) {
          await member.roles
            .remove(roleId, "Arena rank role sync")
            .catch((error) => {
              console.warn(
        ...
        if (!hasRole && shouldHave) {
          await member.roles
            .add(roleId, "Arena rank role sync")
            .catch((error) => {
              console.warn(
