...
const HELP_PAGES = {
  main: {
    ...
    body: [
      ...
      "⚔️ **Battle** — fight, boss, arena, challenge, team preset",
      "‍☠️ **Raid & Team** — raid rooms and saved raid team",
      "🏴‍☠️ **Pirate** — pirate crew, storage, perks, raid, shop",
  ...
  battle: {
    ...
    body: [
      ...
      `\`${PREFIX} add  \` — add card to team`,
      `\`${PREFIX} remove \` — remove card from slot`,
      `\`${PREFIX} remove all\` — clear battle team`,
      "",
      "**Team Preset**",
      `\`${PREFIX} tp\` — view team presets`,
      `\`${PREFIX} tp save <1-3>\` — save current team to preset slot`,
      `\`${PREFIX} tp <1-3>\` — equip saved team preset`,
      `\`${PREFIX} tp remove <1-3>\` — delete saved team preset`,
    ],
  },
  ...
  pirate: {
    label: "Pirate",
    description: "Pirate crew, perks, raid, shop",
    emoji: "🏴‍☠️",
    title: "🏴‍☠️ Pirate Commands",
    body: [
      "**Pirate System**",
      `\`${PREFIX} p\` — show pirate help menu`,
      `\`${PREFIX} p info\` — view your pirate overview`,
      `\`${PREFIX} p create <name>\` — create a pirate crew`,
      `\`${PREFIX} p storage\` — view pirate storage`,
      `\`${PREFIX} p level\` — view pirate level requirement`,
      `\`${PREFIX} p perks\` — view global pirate perks`,
      `\`${PREFIX} p shop\` — view pirate token shop`,
      `\`${PREFIX} p raid\` — view pirate raid bosses`,
      `\`${PREFIX} p lb\` — view weekly pirate leaderboard`,
      "",
      "**Notes**",
      "• Pirate create costs **2,000,000 berries + 250 gems**.",
      "• Pirate perks apply globally to all crew members.",
      "• Pirate raid points reset weekly and reward pirate tokens.",
    ],
  },
