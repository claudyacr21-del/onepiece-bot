...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const targetId = message.mentions.users.first()?.id || parseUserId(args.shift());

    if (message.mentions.users.first()) {
      const mentionIndex = args.findIndex((arg) => String(arg || "").includes(targetId));
    ...
    updatePlayerAtomic(
      ...
      (fresh) => {
        ...
        const cards = Array.isArray(fresh.cards) ? fresh.cards.map((card) => ({ ...card })) : [];
        const card = cards[found.index];

        oldPrestige = clampPrestige(card.raidPrestige);
        const requestedRemove = Math.floor(amount);
        newPrestige = clampPrestige(oldPrestige - requestedRemove);
        realRemoved = oldPrestige - newPrestige;
    ...
    const embed = new EmbedBuilder()
      .setColor(0xe74c3c)
      .setTitle("⭐ Raid Prestige Removed")
      .setDescription(
        [
          `**User:** <@${targetId}>`,
          `**Card:** ${cardName}`,
          ...
          "",
          realRemoved <= 0
            ? "This card already has 0 raid prestige."
            : "Raid prestige reduced successfully.",
        ].join("\n")
      )
