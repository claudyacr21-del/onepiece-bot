...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!userId || !query) {
      return message.reply("Usage: `op removeboost <userId> <boost code or exact name>`");
    }
    ...
    if (idx === -1) {
      return message.reply("Boost card not found.");
    }
    ...
    return message.reply(`Removed boost card \`${removed.displayName || removed.name}\` from \`${userId}\`.`);
  },
};